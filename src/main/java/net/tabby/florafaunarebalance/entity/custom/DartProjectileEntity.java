...
public class DartProjectileEntity extends AbstractArrow {
    ...
    private final Set<MobEffectInstance> effect;
    ...
    public DartProjectileEntity(LivingEntity p_36718_shooter, Level p_36719_level, Item referenceItem, Set<MobEffectInstance> effect, int col) {
    ...
    protected void onHitEntity(@NotNull EntityHitResult entityHitResult){
        ...
        if (target instanceof LivingEntity targetEntity) {
            if (!this.effect.isEmpty()) {
                for (MobEffectInstance entry : effect) {
                    targetEntity.addEffect(entry);
                }
            }
        }
    }
