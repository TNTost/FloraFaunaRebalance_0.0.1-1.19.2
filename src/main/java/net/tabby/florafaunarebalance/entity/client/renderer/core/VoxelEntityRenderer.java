...
public abstract class VoxelEntityRenderer<T extends Entity, M extends EntityModel<T>> extends EntityRenderer<T> implements RenderLayerParent<T, M> {
    ...
    public void render(T entity, float p_115309_, float subTicks, PoseStack poseStack, MultiBufferSource source, int p_115313_) {
        poseStack.pushPose();
        ...
        this.model.renderToBuffer(poseStack, source.getBuffer(rendertype), p_115313_, getOverlayCoords(entity, 0.0f), 1.0F, 1.0F, 1.0F, 0.15f); //flag1 ? 0.15F : 1.0F

        if (!entity.isSpectator()) {
            for(RenderLayer<T, M> renderlayer : this.layers) {
                renderlayer.render(poseStack, source, p_115313_, entity, 0.0f, 0.0f, subTicks, f7, f2, f6);
            }
        }

        poseStack.popPose();
        super.render(entity, p_115309_, subTicks, poseStack, source, p_115313_);
    }
